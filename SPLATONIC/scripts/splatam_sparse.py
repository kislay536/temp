...
from utils.common_utils import seed_everything, save_params_ckpt, save_params, ModuleTimer
...
from utils.draw_utils import CameraVisualizer
from utils.mask_utils import generate_random_mask, get_pixel_info, adaptive_random_sampling
from utils.loss_utils import calc_ssim_shuffled_packed
...
def get_sparse_loss(params, curr_data, variables, iter_time_idx, loss_weights, use_sil_for_loss,
    ...
    if tracking: 
        pixel_range, pixel_coords = get_pixel_info(pixel_mask)
    elif mapping:
        pixel_range, pixel_coords = get_pixel_info(pixel_mask, tile_size=4)
    ...
    #         pixel_range, pixel_coords = get_pixel_info(pixel_mask, tile_size=4)
    # else:
    #     pixel_range, pixel_coords = pixel_info[0], pixel_info[1]
    ...
    rgb_renderer = TrackRenderer(raster_settings=curr_data['cam']) if tracking else MapRenderer(raster_settings=curr_data['cam'])
    depth_renderer = TrackRenderer(raster_settings=curr_data['cam']) if tracking else MapRenderer(raster_settings=curr_data['cam'])
    rgb_timer = ModuleTimer(rgb_renderer, name="rgb_sparse_renderer")
    depth_timer = ModuleTimer(depth_renderer, name="depth_sparse_renderer")

    # RGB Rendering
    rendervar['means2D'].retain_grad()
    im, radius, _, = rgb_renderer(**rendervar)
    variables['means2D'] = rendervar['means2D']  # Gradient only accum from colour render for densification

    # Depth & Silhouette Rendering
    depth_sil, _, _, = depth_renderer(**depth_sil_rendervar)
    ...
    return loss, variables, weighted_losses, (rgb_timer, depth_timer)
...
def get_loss(params, curr_data, variables, iter_time_idx, loss_weights, use_sil_for_loss,
    ...
    rgb_renderer = Renderer(raster_settings=curr_data['cam'])
    depth_renderer = Renderer(raster_settings=curr_data['cam'])
    rgb_timer = ModuleTimer(rgb_renderer, name="rgb_sparse_renderer")
    depth_timer = ModuleTimer(depth_renderer, name="depth_sparse_renderer")
...
def rgbd_slam(config: dict):
    ...
    vis = CameraVisualizer(width=720, height=480)
    FLIP = 4
    
    traj_dir = os.path.join(eval_dir, f"camera_trajectory")
    os.makedirs(traj_dir, exist_ok=True)
    rendering_dir = os.path.join(eval_dir, f"renderings")
    ...
    for time_idx in tqdm(range(checkpoint_time_idx, num_frames)):
        ...
        if time_idx > 0 and not config['tracking']['use_gt_poses']:
            ...
            while True:
                ...
                loss, variables, losses, timers = get_sparse_loss(params, tracking_curr_data, variables, iter_time_idx, config['tracking']['loss_weights'],
                                                   config['tracking']['use_sil_for_loss'], config['tracking']['sil_thres'],
                                                   config['tracking']['use_l1'], config['tracking']['ignore_outlier_depth_loss'], tracking=True, 
                                                   plot_dir=eval_dir, visualize_tracking_loss=config['tracking']['visualize_tracking_loss'],
                                                   tracking_iteration=iter, pixel_mask=pixel_mask, pixel_info=(pixel_range, pixel_coords))
                if config['use_wandb']:
                    # Report Loss
                ...
                tracking_iter_time_sum += timers[0].get_full_time() + timers[1].get_full_time()
        ...
        traj_img = vis.visualize(curr_c2w.cpu().numpy(), current_frame=time_idx)
        save_path = os.path.join(traj_dir, f"{time_idx:04d}.png")
        cv2.imwrite(save_path, cv2.cvtColor(traj_img, cv2.COLOR_RGB2BGR))
        ...
        if time_idx == 0 or (time_idx+1) % config['map_every'] == 0:
            ...
            for iter in range(num_iters_mapping):
                ...
                if selected_rand_keyframe_idx == -1:
                    ...
                    iter_unseen = keyframe_list[selected_rand_keyframe_idx]['novelty'] 
                ...
                # Loss for current frame
                if counter == 0:
                    loss, variables, losses, timers = get_loss(params, iter_data, variables, iter_time_idx, config['mapping']['loss_weights'],
                    ...
                    # pixel_mask, _, _ = generate_random_mask(iter_color.shape[1:], tile_size=4, device='cuda')
                    if iter_unseen is not None:
                        pixel_mask = pixel_mask | iter_unseen.reshape(pixel_mask.shape)
                    loss, variables, losses, timers = get_sparse_loss(params, iter_data, variables, iter_time_idx, config['mapping']['loss_weights'],
                                config['mapping']['use_sil_for_loss'], config['mapping']['sil_thres'],
                ...
                with torch.no_grad():
                    ...
                    if config['report_iter_progress']:
                        ...
                        progress_bar.update(1)
                # Update the runtime numbers
                mapping_iter_time_sum += timers[0].get_full_time() + timers[1].get_full_time()
            ...
            # draw rendering image
            rendering_image = variables['image'].detach().cpu().numpy().transpose(1, 2, 0)
            save_path = os.path.join(rendering_dir, f"{time_idx:04d}.png")
            cv2.imwrite(save_path, cv2.cvtColor((rendering_image*255).astype(np.uint8), cv2.COLOR_RGB2BGR))
        ...
        if ((time_idx == 0) or ((time_idx+1) % config['keyframe_every'] == 0) or \
            ...
            with torch.no_grad():
                ...
                curr_w2c[:3, 3] = curr_cam_tran
                # Initialize Keyframe Info
                curr_keyframe = {'id': time_idx, 'est_w2c': curr_w2c, 'color': color, 'depth': depth, 'novelty': variables['novelty'], 'counter': 0}
                # Add to keyframe list
                keyframe_list.append(curr_keyframe)
    ...
    print(f"\nAverage Tracking/Iteration Time: {tracking_iter_time_avg} ms")
    print(f"Average Tracking/Frame Time: {tracking_frame_time_avg} s")
    print(f"Average Mapping/Iteration Time: {mapping_iter_time_avg} ms")
    print(f"Average Mapping/Frame Time: {mapping_frame_time_avg} s")
    if config['use_wandb']:
        wandb_run.log({"Final Stats/Average Tracking Iteration Time (ms)": tracking_iter_time_avg,

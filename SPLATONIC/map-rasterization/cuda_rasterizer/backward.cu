	...
	float* __restrict__ dL_dcolors)
{
	...
	int progress = block.thread_rank();
	...
	for (int i = 0; i < rounds; i++, progress += BLOCK_SIZE)
	{
		...
		if (in_range)
		{
			coll_id = point_list[range.y - progress - 1].x;
		...
		// intra warp reduce for inv alpha
		cur_T = inv_alpha;
		...
		// currently, last_T is the accumulated T of previous block, collected_T records warp_reduced inv_alpha multiplys
		block.sync();
		for(int j = 0; j < warp_idx; j++)
		{
			last_T *= collected_T[j];
		}
		if(warp_idx == NUM_WARPS - 1 && lane < NUM_WARPS) collected_T[lane] = tmp * last_T;
		cur_T *= last_T; // cur_T = T_i

		// 计算 color 累加
		float dchannel_dcolor = alpha * cur_T;
		for (int ch = 0; ch < C; ch++)
		{
			// color gradient
			...
			// intra warp reduce for (T_i * alpha_i * c_i)
			for (int offset = 1; offset < 32; offset *= 2)
			{
				tmp = __shfl_up_sync(~0, cur_accum[ch], offset);
				if (lane >= offset) cur_accum[ch] += tmp;
			}

			last_accum[ch] = collected_accum[ch][warp_idx];
			tmp = __shfl_sync(~0, cur_accum[ch], 31);
			if (lane == 0) collected_accum[ch][warp_idx] = tmp;
			// the prifix sum is self-excluded
			cur_accum[ch] = __shfl_up_sync(~0, cur_accum[ch], 1);
			...
			if(warp_idx == NUM_WARPS - 1 && lane < NUM_WARPS) collected_accum[ch][lane] = tmp + last_accum[ch];
			cur_accum[ch] += last_accum[ch]; // cur_accum = \sum_{j=i+1}^{N} T_j * alpha_j * c_j
		}
...
void BACKWARD::render(
	const int num_pixel, const dim3 block,
	const uint2* ranges,
	...
	float* dL_dcolors)
{
	renderCUDA<NUM_CHANNELS> << <num_pixel, block >> >(
		ranges,
		point_list,
		pixel_coords,
		W, H,

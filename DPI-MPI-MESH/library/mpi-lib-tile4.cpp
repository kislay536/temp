...
void mpi_packet_serve(int flag)
{
    ...
    mpi_send_response(resp_output, 0, 5, flag);
}
...
int main()
{
    ...
    while (!sim_end)
    {
        ...
        MPI_Iprobe(0, 599, MPI_COMM_WORLD, &flag, &status);

        if (flag)
        {
            int dummy;
            // MPI_Recv( void* buf , MPI_Count count , MPI_Datatype datatype , int source , int tag , MPI_Comm comm , MPI_Status* status);
            MPI_Recv(&dummy, 1, MPI_INT, 0, 599, MPI_COMM_WORLD, &status);
            std::cout << "Shutdown signal received!" << std::endl;
            sim_end = true;
            break;
        }

        MPI_Iprobe(0, tile4, MPI_COMM_WORLD, &req_available, &status);
        std::cout << "==== ==== alu_tile_noc Waiting for Requests!!! ==== ====" << std::endl;
        if (req_available)
            mpi_packet_serve(tile4); // serve other requests
    }

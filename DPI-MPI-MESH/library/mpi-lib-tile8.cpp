...
void mpi_packet_serve(int flag)
{
    ...
    mpi_send_response(resp_output, 0, 9, flag);
}
...
int main()
{
    ...
    while (!sim_end)
    {
        ...
        if (flag)
        {
            ...
            MPI_Recv(&dummy, 1, MPI_INT, 0, 999, MPI_COMM_WORLD, &status);
            std::cout << "Shutdown signal received!" << std::endl;
            sim_end = true;
            break;
        }

        MPI_Iprobe(0, tile8, MPI_COMM_WORLD, &req_available, &status);
        std::cout << "==== ==== alu_tile_noc Waiting for Requests!!! ==== ====" << std::endl;
        if (req_available)
            mpi_packet_serve(tile8); // serve other requests
    }

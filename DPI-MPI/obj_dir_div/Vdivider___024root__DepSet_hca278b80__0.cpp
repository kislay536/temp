...
VL_INLINE_OPT void Vdivider___024root___ico_sequent__TOP__0(Vdivider___024root* vlSelf) {
    ...
    auto& vlSelfRef = std::ref(*vlSelf).get();
    // Body
    vlSelfRef.result = ((0ULL == vlSelfRef.b) ? 0ULL
...
#endif
            VL_FATAL_MT("divider.v", 1, "", "Input combinational region did not converge.");
        }
...
#endif
            VL_FATAL_MT("divider.v", 1, "", "NBA region did not converge.");
        }
...
#endif
                VL_FATAL_MT("divider.v", 1, "", "Active region did not converge.");
            }

...
VL_INLINE_OPT void Vmultiplier___024root___ico_sequent__TOP__0(Vmultiplier___024root* vlSelf) {
    ...
    vlSelfRef.ack_to_sub = 0U;
    vlSelfRef.ack_to_div = 0U;
    vlSelfRef.working = 0U;
    if (vlSelfRef.start) {
        if ((((IData)(vlSelfRef.ack_in_add) & (IData)(vlSelfRef.ack_in_sub)) 
        ...
        vlSelfRef.ack_to_sub = (1U & (~ (IData)(vlSelfRef.working)));
        vlSelfRef.ack_to_div = (1U & (~ (IData)(vlSelfRef.working)));
    } else {
        if (vlSelfRef.add_working) {
            vlSelfRef.ack_to_add = 1U;
        }
        if (vlSelfRef.sub_working) {
            vlSelfRef.ack_to_sub = 1U;
        }
...
#endif
            VL_FATAL_MT("multiplier.v", 2, "", "Input combinational region did not converge.");
        }
...
#endif
            VL_FATAL_MT("multiplier.v", 2, "", "NBA region did not converge.");
        }
...
#endif
                VL_FATAL_MT("multiplier.v", 2, "", "Active region did not converge.");
            }
...
void Vmultiplier___024root___eval_debug_assertions(Vmultiplier___024root* vlSelf) {
    ...
    if (VL_UNLIKELY(((vlSelfRef.ack_in_sub & 0xfeU)))) {
        Verilated::overWidthError("ack_in_sub");}
    ...
    if (VL_UNLIKELY(((vlSelfRef.sub_working & 0xfeU)))) {
        Verilated::overWidthError("sub_working");}
